...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for (int a=0; a<2; a++) {
            ...
            for (int i = 0; i < 2; i++) {
                if (grid[i][0] == test && grid[i][1] == test && grid[i][2] == test) {
                    result = test + "Win!";
                }
            }
            ...
            for (int i = 0; i < 2; i++) {
                if (grid[0][i] == test && grid[1][i] == test && grid[2][i] == test) {
                    result = test + " Win!";
                }

            }

            if (grid[0][0] == test && grid[1][1] == test && grid[2][2] == test || grid[2][0] == test && grid[1][1] == test && grid[0][2] == test) {
                result = test + " Win!!!!";
            }
